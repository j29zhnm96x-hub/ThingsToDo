...
export async function renderProjectDetail(ctx, projectId) {
  ...
  const list = renderTodoList({
    ...
    onMoveUp: async (todo) => swapWithNeighbor({ ctx, todo, direction: -1, containerProjectId: projectId }),
    onMoveDown: async (todo) => swapWithNeighbor({ ctx, todo, direction: 1, containerProjectId: projectId })
  });
...
async function swapWithNeighbor({ ctx, todo, direction, containerProjectId }) {
  ...
  await renderProjectDetail(ctx, containerProjectId);
}
